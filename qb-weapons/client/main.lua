...
RegisterNetEvent('weapon:client:AddAmmo', function(type, amount, itemData)
    ...
    if CurrentWeaponData ~= nil then
        if QBCore.Shared.Weapons[weapon]["name"] ~= "weapon_unarmed" and QBCore.Shared.Weapons[weapon]["ammotype"] == type:upper() then
            ...
            if (total + retval) <= (retval + 249) then
                QBCore.Functions.Progressbar("taking_bullets", "Loading bullets..", math.random(500, 1000), false, true, {
                    ...
                    QBCore.Functions.Notify("Cancelled action.", "error")
                end)
            else
                QBCore.Functions.Notify("Your weapon is already loaded to its maximum capacity.", "error")
            end
        else
